...
class ApiController extends Controller
{
    ...
    public function get_system_settings(Request $request)
    {
        ...
        foreach ($result as $row) {
            ...
            } elseif ($row->type == 'company_logo') {

                $tempRow[$row->type] = url('/assets/images/logo/logo.png');
            } elseif ($row->type == 'web_logo' || $row->type == 'web_placeholder_logo' || $row->type == 'app_home_screen' || $row->type == 'web_footer_logo' || $row->type == 'placeholder_logo' || $row->type == 'favicon_icon') {
    ...
    public function getPaymentTransactionDetails(Request $request)
    {
        try {
            ...
            if (count($result)) {
                ApiResponseService::successResponse('Data Fetch Successfully', $result, ['total' => $total]);
            } else {
            ...
            if ($chat) {
                ...
                $chat->map(function ($chat) use ($chat_message_type, $currentUser) {
                    if (! empty($chat->audio)) {
                        ...
                        $chat_message_type = 'file_and_text';
                    } elseif (empty($chat->file) && ! empty($chat->message) && empty($chat->audio)) {
                        $chat_message_type = 'text';
                    } else {
                        $chat_message_type = 'text';
                    }
    ...
    public function update_property_status(Request $request)
    {
        ...
        try {
            ...
            } elseif ($property->getRawOriginal('propery_type') != 0 && $property->getRawOriginal('propery_type') != 1) {
        ...
        try {
            ...
            if (collect($result)->isNotEmpty()) {
                ...
                foreach ($result as $row) {
                    ...
                    } elseif ($row->type == 'app_home_screen' || $row->type == 'placeholder_logo') {
                        $settingsData[$row->type] = url('/assets/images/logo/').'/'.$row->data;
                    } elseif ($row->type == 'verification_required_for_user') {
                        // Change Value to Bool
                        $settingsData[$row->type] = $row->data == 1 ? true : false;
    ...
    public function getOtp(Request $request)
    {
        ...
        try {
            ...
            return response()->json([
                ...
                'otp' => $otp, // Remove this line in production for security reasons
            ]);
    ...
    public function getPropertyList(Request $request)
    {
        try {
            ...
            elseif ($request->has('most_liked') && $request->most_liked == 1) {
    ...
    public function getAddedProjects(Request $request)
    {
        try {
            ...
            if ($request->filled('id') || $request->filled('slug_id')) {
                ...
                ApiResponseService::successResponse('Data Fetched Successfully', $data, ['similar_projects' => $getSimilarProjects]);
            } else {
    ...
    public function getProjects(Request $request)
    {
        try {
            ...
            $projectsQuery = Projects::where(['request_status' => 'approved', 'status' => 1])
                ...
                ->when($request->filled('get_featured') && $request->get_featured == 1, function ($query) {
    ...
    */
    ...
    public function getUnsplashData($cityData)
    {
    ...
    public function roundArrayValues($array, $pointsValue)
    {
    ...
    public function mortgageCalculation($loanAmount, $downPayment, $interestRate, $loanTermYear, $showAllDetails)
    {
        if ($downPayment > 0) {
     ...
     * @param  bool  $homepageLocationDataAvailable
